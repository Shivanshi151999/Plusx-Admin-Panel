  ...
  const fetchDetails = () => {
    ...
    postRequestWithToken('shop-data', obj, (response) => {
      if (response.code === 200) {
        ...
        setBrands(initialSelectedBrands);

        const initialSelectedServices = (data?.services.split(", ") || []).map(item => ({
          label: item,
          value: item
        }));
        setServices(initialSelectedServices);

        const transformedLocation = (response?.result?.location || []).map(item => ({
          label: item,
          value: item
        }));
